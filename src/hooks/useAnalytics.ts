...
import { callsApi, smsApi } from "@/lib/api-client";
...
export interface AnalyticsData {
  ...
  dailyCalls: DailyMessageCount[];
  portActivity: PortActivity[];
  hourlyDistribution: HourlyDistribution[];
  hourlyCalls: HourlyDistribution[];
  totalMessages: number;
  totalCalls: number;
  averagePerDay: number;
  callsAveragePerDay: number;
  busiestPort: number | null;
  callsBusiestPort: number | null;
  peakHour: number | null;
  callsPeakHour: number | null;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AnalyticsData> => {
      const fallback: AnalyticsData = {
        ...
        totalCalls: 0,
        averagePerDay: 0,
        callsAveragePerDay: 0,
        busiestPort: null,
        callsBusiestPort: null,
        peakHour: null,
        callsPeakHour: null,
        extensionBreakdown: [],
      };

      const [statsJson, smsJson, callsJson] = await Promise.all([
        callsApi.statistics().catch(() => ({ data: {} })),
        smsApi.messages({ limit: 2000 }).catch(() => ({ data: [] })),
        callsApi.records({ limit: 2000 }).catch(() => ({ data: [] })),
      ]);

      if (!statsJson) throw new Error("Failed to fetch analytics");

      const rawStats = (statsJson as any)?.data || statsJson || {};
      // smsApi.messages uses apiFetch → returns the raw array directly
      const smsMessages: SmsMessage[] = Array.isArray(smsJson)
        ? (smsJson as SmsMessage[])
        : Array.isArray((smsJson as any)?.data) ? (smsJson as any).data : [];
      // callsApi.records uses apiFetchFull → returns { success, data: [...], pagination: {} }
      const callRecords: CallRecord[] = Array.isArray((callsJson as any)?.data)
        ? (callsJson as any).data
        : Array.isArray(callsJson) ? (callsJson as CallRecord[]) : [];
      ...
      const callsHourMap = new Map<number, number>();
      dayKeys.forEach(k => callsDayMap.set(k, 0));
      for (let h = 0; h < 24; h++) callsHourMap.set(h, 0);
      callsInRange.forEach((call) => {
        const date = parseDate(call.start_time || undefined);
        if (!date) return;
        const dayKey = date.toISOString().slice(0, 10);
        callsDayMap.set(dayKey, (callsDayMap.get(dayKey) || 0) + 1);
        const hour = date.getHours();
        callsHourMap.set(hour, (callsHourMap.get(hour) || 0) + 1);
      });
      const dailyCalls = dayKeys.map((key) => ({ date: key, count: callsDayMap.get(key) || 0 }));
      const hourlyCalls = Array.from(callsHourMap.entries()).map(([hour, count]) => ({ hour, count }));

      const totalCalls = callsInRange.length;
      const callsAveragePerDay = days > 0 ? totalCalls / days : 0;
      const callsPortMap = new Map<number, number>();
      callsInRange.forEach((call) => {
        const port = Number(call.sim_port);
        if (!Number.isNaN(port) && port > 0) callsPortMap.set(port, (callsPortMap.get(port) || 0) + 1);
      });
      const callsPeakHour = hourlyCalls.length
        ? [...hourlyCalls].sort((a, b) => b.count - a.count)[0].hour
        : null;
      const callsBusiestPort = callsPortMap.size
        ? [...callsPortMap.entries()].sort((a, b) => b[1] - a[1])[0][0]
        : null;
