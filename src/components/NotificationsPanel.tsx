...
export const NotificationsPanel = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const errorMsg = result.data?.details || result.error || "Failed to generate report";
  ...
  const handleTestSms = async () => {
    setTestingSms(true);
    try {
      const result = await apiCall('/api/test-sms', { method: 'POST' });
      if (result.success) {
        toast.success(`Test SMS sent to ${result.data?.details || ''}`);
      } else {
  ...
  const handleTestEmail = async () => {
    ...
    try {
      const result = await apiCall('/api/test-email', { method: 'POST' });
      if (result.success) {
        toast.success(`Test email sent to ${result.data?.details || ''}`);
      } else {
        const msg = result.data?.details || result.error || 'Email test failed';
        toast.error(msg);
      }
    } catch (e) {
  ...
  const saveNotifTemplate = async (eventType: string, text: string) => {
    const result = await apiCall(`/api/notification-templates/${eventType}`, {
      method: 'POST',
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="sms" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="pt-6 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Select which extensions trigger Call Auto-SMS. Extensions not selected will be ignored.
                  <strong className="text-foreground"> At least one must be selected for auto-SMS to fire.</strong>
                </p>
                <div className="p-4 rounded-lg bg-muted/30 border border-border/30 space-y-2">
                  {extensions.length > 0 ? (
                    extensions.map((ext) => (
                  ...
                  {allowedExtensions.length === 0 && extensions.length > 0 && (
                    <Alert className="border-amber-500/50 bg-amber-500/5">
                      ...
                      <AlertDescription className="text-xs text-amber-700">No extensions selected — auto-SMS will not fire even when enabled. Select at least one extension to monitor.</AlertDescription>
                    </Alert>
                  )}
                </div>
              </div>
        ...
        <TabsContent value="telegram" className="space-y-4">
          ...
          <div className={`space-y-4${!alertConfig.enabled ? " opacity-50 pointer-events-none" : ""}`}>
          <Card className="border-border/50">
        ...
        <TabsContent value="email" className="space-y-4">
          ...
          <div className={`space-y-4${!alertConfig.email_enabled ? " opacity-50 pointer-events-none" : ""}`}>
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                <Bell className="w-4 h-4" />
