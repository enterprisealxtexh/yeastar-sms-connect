...
const useAgentRatings = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const json = await apiFetch<any>('/api/agent-ratings');
      return ((json as any).data || json || []) as AgentRating[];
    },
  });
};
...
const useSubmitRating = () => {
  ...
  return useMutation({
    mutationFn: async (input: { agent_id: string; rating: number; comment?: string }) => {
      const result = await apiCall('/api/agent-ratings', {
        method: "POST",
        body: JSON.stringify({
      ...
      apiCall('/api/notify/shift-change', {
        method: "POST",
        body: JSON.stringify({
...
export const AgentShiftRating = () => {
  ...
  const sendDigest = useMutation({
    mutationFn: async () => {
      const result = await apiCall('/api/notify/shift-change', {
        method: "POST",
        body: JSON.stringify({ action: "weekly_rating_digest" }),
      });
      if (!result.success) throw new Error(result.error || "Failed to send digest");
      return result;
    },
