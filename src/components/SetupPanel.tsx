...
export const SetupPanel = forwardRef<SetupPanelRef>(function SetupPanel(_, ref) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (smsData) {
          // apiFetch already extracts .data — smsData is the array directly
          if (Array.isArray(smsData)) setSmsRecipients(smsData);
        }
      } catch (err) {
  ...
  useImperativeHandle(ref, () => ({ save: handleSave, isSaving }), [config, isSaving]);
  ...
  const handleAddPhone = async () => {
    ...
    try {
      const result = await apiCall('/api/sms-report-recipients', {
        method: "POST",
      ...
      if (result.success) {
        ...
        setNewPhoneNumber("");
        const d = await apiFetch<SmsRecipient[]>('/api/sms-report-recipients');
        setSmsRecipients(Array.isArray(d) ? d : []);
      } else {
  ...
  const handleRemovePhone = async (phoneNumber: string) => {
    ...
    try {
      const result = await apiCall(`/api/sms-report-recipients/${encodeURIComponent(phoneNumber)}`, {
        method: "DELETE",
      });
      if (result.success) {
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="border-border/50 h-full">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-1">
              <Label className="text-xs">
                From Address <span className="text-muted-foreground font-normal">(optional — must be a valid email)</span>
              </Label>
              <Input
                type="email"
              ...
              <p className="text-[10px] text-muted-foreground">
                Must be a valid email address. Defaults to SMTP username if empty.
              </p>
            </div>
          </CardContent>
        </Card>
