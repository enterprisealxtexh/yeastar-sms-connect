...
export const GatewaySettingsForm = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success("TG400 gateway configuration has been updated.");
      
      setConnectionStatus('idle');
    } catch (error) {
      console.error('[GatewaySettingsForm] Save error:', error);
      toast.error(error instanceof Error ? error.message : "Failed to save gateway settings");
    }
  };
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const result = await apiFetch<any>('/api/gateway-test');
      
      if (result.success) {
        setConnectionStatus('success');
        toast.success(`✅ Connected to TG400 at ${gateway_ip}:${api_port} - Authentication successful`);
      } else {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to test connection to TG400 Gateway.");
    } finally {
      setIsTesting(false);
    }
  };
