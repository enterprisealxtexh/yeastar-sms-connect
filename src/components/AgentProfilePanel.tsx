  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      const [callsJson, portsJson, smsJson] = await Promise.all([
        apiFetch<any>(`/api/call-records?extension=${encodeURIComponent(extension!)}&start_time_from=${encodeURIComponent(since)}&limit=10000`),
        apiFetch<any>('/api/sim-ports'),
        apiFetch<any>(`/api/sms-messages?limit=10000&since=${encodeURIComponent(since)}`),
      ]);

      const calls = ((callsJson as any).data || callsJson || []) as { status: string; direction: string; talk_duration: number }[];
