...
const useMyDailyReport = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const json = await apiFetch<any>(`/api/call-records?start_time_from=${encodeURIComponent(start)}&start_time_to=${encodeURIComponent(end)}&limit=10000`);
...
export const UserProfilePanel = () => {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    const json = await apiCall('/api/users/change-password', {
      method: 'PUT',
  ...
  const handleChangePin = async (e: React.FormEvent) => {
    ...
    const json = await apiCall('/api/users/change-pin', {
      method: 'PUT',
