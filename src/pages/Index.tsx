...
const Index = () => {
  ...
  // Dashboard tab data
  const { data: messages = [], isLoading: messagesLoading } = useSmsMessages(50, 'received', activeTab === "dashboard" || activeTab === "messages");
  const { data: logs = [], isLoading: logsLoading } = useActivityLogs(50, activeTab === "dashboard");
  const { data: stats, isLoading: statsLoading } = useDashboardStats(activeTab === "dashboard");
  
  // Calls tab data
  const { data: callsResponse, isLoading: callsLoading } = useCallRecords(
    callRecordsPage, 
    50, 
  ...
  const { data: callStats, isLoading: callStatsLoading } = useCallStats(viewerExtForStats, activeTab === "calls");
  const { data: allTimeCallStats, isLoading: allTimeCallStatsLoading } = useAllTimeCallStats(viewerExtForStats, activeTab === "calls");
  
  const { config: pbxConfig } = usePbxConfig(activeTab === "dashboard" && isAdmin);
  const { data: pbxStatus } = usePbxStatus(activeTab === "dashboard" && isAdmin);
  const { data: gatewayStatus } = useGatewayStatus(activeTab === "dashboard" && isAdmin);
