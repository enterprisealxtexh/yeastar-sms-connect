...
class SMSDatabase {
  ...
  createTables() {
    ...
    // SMS Pending Queue table - persists delayed call auto-SMS across restarts
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_sms_pending_queue_status ON sms_pending_queue(status, scheduled_at);
    `);
    ...
    // Customer Rating Settings (single-row, managed by super_admin)
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS customer_rating_settings (
        id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
        enabled BOOLEAN DEFAULT 0,
        ...
        extension_policies_json TEXT DEFAULT '[]',
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );
    `);

    // One-time rating links generated by the system and sent to customers
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS customer_rating_links (
        id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
        token TEXT UNIQUE NOT NULL,
        phone_number TEXT NOT NULL,
        source TEXT NOT NULL CHECK (source IN ('auto_reply', 'call_auto_sms')),
        extension TEXT,
        call_record_id TEXT,
        possible_agent_ids_json TEXT DEFAULT '[]',
      ...
      CREATE INDEX IF NOT EXISTS idx_customer_rating_links_token ON customer_rating_links(token);
      CREATE INDEX IF NOT EXISTS idx_customer_rating_links_phone ON customer_rating_links(phone_number);
      CREATE INDEX IF NOT EXISTS idx_customer_rating_links_status ON customer_rating_links(status, expires_at);
    `);

    // Submitted customer ratings
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS customer_rating_submissions (
        id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
        link_id TEXT NOT NULL,
        token TEXT NOT NULL,
        phone_number TEXT NOT NULL,
        overall_rating INTEGER NOT NULL CHECK (overall_rating >= 1 AND overall_rating <= 5),
        recommend_rating INTEGER CHECK (recommend_rating >= 1 AND recommend_rating <= 5),
        comments TEXT,
        ...
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (link_id) REFERENCES customer_rating_links(id) ON DELETE CASCADE
      );

      CREATE INDEX IF NOT EXISTS idx_customer_rating_submissions_agent ON customer_rating_submissions(agent_id);
      CREATE INDEX IF NOT EXISTS idx_customer_rating_submissions_time ON customer_rating_submissions(submitted_at DESC);
    `);
    ...
    const customerRatingSettingsCount = this.db.prepare('SELECT COUNT(*) as cnt FROM customer_rating_settings').get().cnt;
    if (customerRatingSettingsCount === 0) {
      this.db.prepare(`
        INSERT INTO customer_rating_settings (
          enabled,
          company_name,
          company_icon_url,
          public_base_url,
      ...
      `).run(
        ...
        1,
        JSON.stringify([
          { id: 'q1', text: 'How would you rate our service?', type: 'stars', required: true }
        ]),
        JSON.stringify([])
      );
    }
  ...
  runMigrations() {
    try {
      ...
      // Migration: Make sms_messages.sim_port nullable (legacy DBs have NOT NULL constraint
      // which breaks INSERT for sent/auto-reply SMS records that have no gsm_span/sim_port)
      const smsTableInfo2 = this.db.prepare(`PRAGMA table_info(sms_messages)`).all();
      const simPortCol = smsTableInfo2.find(col => col.name === 'sim_port');
      if (!simPortCol) {
        // Column missing entirely — add it as nullable (DBs from the intermediate schema)
        logger.info('🔄 Migrating: Adding sim_port column to sms_messages...');
        try {
          this.db.exec(`ALTER TABLE sms_messages ADD COLUMN sim_port INTEGER CHECK (sim_port IS NULL OR (sim_port >= 1 AND sim_port <= 4))`);
          this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_sim_port ON sms_messages(sim_port)`);
          logger.info('✅ Migration complete: sms_messages.sim_port added');
        } catch (e) {
          if (!e.message.includes('duplicate column name')) {
            logger.warn(`⚠️  Could not add sms_messages.sim_port: ${e.message}`);
          }
        }
      } else if (simPortCol.notnull === 1) {
        logger.info('🔄 Migrating: Making sms_messages.sim_port nullable...');
        try {
          const migrateSimPortNullable = this.db.transaction(() => {
            this.db.exec(`
            ...
            `);
            this.db.exec(`
              INSERT INTO sms_messages_v2 (id, external_id, sender_number, message_content, received_at, sim_port, gsm_span, status, direction, category, created_at, updated_at)
              SELECT id, external_id, sender_number, message_content, received_at, sim_port, gsm_span, status, direction, category, created_at, updated_at FROM sms_messages
            `);
            this.db.exec(`DROP TABLE sms_messages`);
            this.db.exec(`ALTER TABLE sms_messages_v2 RENAME TO sms_messages`);
            this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_sim_port ON sms_messages(sim_port)`);
            this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_status ON sms_messages(status)`);
            this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_received_at ON sms_messages(received_at DESC)`);
            this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_sender ON sms_messages(sender_number)`);
            this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_gsm_span ON sms_messages(gsm_span)`);
            this.db.exec(`CREATE INDEX IF NOT EXISTS idx_sms_direction ON sms_messages(direction)`);
          });
          migrateSimPortNullable();
          logger.info('✅ Migration complete: sms_messages.sim_port is now nullable');
        } catch (e) {
          logger.warn(`⚠️  Could not migrate sms_messages.sim_port to nullable: ${e.message}`);
        }
      }
      ...
      // Migration: Add allowed_extensions to auto_reply_config
      try {
      ...
      // Migration: Add sms_pending_queue table if missing
      try {
        this.db.exec(`
          CREATE TABLE IF NOT EXISTS sms_pending_queue (
            id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
  ...
  // ========== SMS PENDING QUEUE METHODS ==========

  enqueuePendingSms(caller_number, message, scheduled_at) {
    try {
      const iso = scheduled_at instanceof Date ? scheduled_at.toISOString() : scheduled_at;
      this.db.prepare(`
        INSERT INTO sms_pending_queue (caller_number, message, scheduled_at) VALUES (?, ?, ?)
      `).run(caller_number, message, iso);
      return true;
    } catch (error) {
      console.error('Error enqueueing pending SMS:', error.message);
      return false;
    }
  }
  ...
  getDueSmsItems() {
    try {
      ...
      console.error('Error getting due SMS items:', error.message);
      return [];
    }
  }

  markPendingSmsProcessed(id, success) {
    try {
      this.db.prepare(`
        UPDATE sms_pending_queue SET status = ?, attempts = attempts + 1 WHERE id = ?
      `).run(success ? 'sent' : 'failed', id);
      return true;
    } catch (error) {
  ...
  hasPendingSms(caller_number) {
    try {
      const result = this.db.prepare(`
        SELECT COUNT(*) as count FROM sms_pending_queue WHERE caller_number = ? AND status = 'pending'
      `).get(caller_number);
      return result.count > 0;
    } catch (error) {
      console.error('Error checking pending SMS:', error.message);
      return false;
    }
  }

  cleanOldPendingQueue(daysOld = 7) {
    try {
      this.db.prepare(`
        DELETE FROM sms_pending_queue WHERE status != 'pending' AND created_at < datetime('now', '-' || ? || ' days')
      `).run(daysOld);
    } catch (error) {
      console.error('Error cleaning old pending queue:', error.message);
    }
  }
  ...
  // Customer Rating Settings + Links + Analytics
  getCustomerRatingSettings() {
    try {
      const row = this.db.prepare('SELECT * FROM customer_rating_settings LIMIT 1').get();
      if (!row) return null;

      let questions = [];
      let extensionPolicies = [];
      try { questions = JSON.parse(row.questions_json || '[]'); } catch (_) {}
      try { extensionPolicies = JSON.parse(row.extension_policies_json || '[]'); } catch (_) {}

      return {
        ...row,
        enabled: !!row.enabled,
        include_recommendation: !!row.include_recommendation,
        link_valid_hours: Number(row.link_valid_hours || 24),
        questions,
        extension_policies: extensionPolicies,
      };
    } catch (error) {
      console.error('Error getting customer rating settings:', error.message);
      return null;
    }
  }

  saveCustomerRatingSettings(settings = {}) {
    try {
      const current = this.getCustomerRatingSettings() || {};
      const normalized = {
        ...
        include_recommendation: settings.include_recommendation !== undefined
          ? (settings.include_recommendation ? 1 : 0)
          : (current.include_recommendation ? 1 : 0),
        questions_json: JSON.stringify(Array.isArray(settings.questions) ? settings.questions : (current.questions || [])),
        extension_policies_json: JSON.stringify(Array.isArray(settings.extension_policies) ? settings.extension_policies : (current.extension_policies || [])),
      };

      const existing = this.db.prepare('SELECT id FROM customer_rating_settings LIMIT 1').get();
      if (existing) {
        this.db.prepare(`
          UPDATE customer_rating_settings
          SET enabled = ?,
        ...
        `).run(
          ...
          normalized.company_icon_url,
          normalized.public_base_url,
          normalized.link_valid_hours,
          normalized.trigger_source,
          normalized.include_recommendation,
          normalized.questions_json,
          normalized.extension_policies_json,
          existing.id
        );
      } else {
        this.db.prepare(`
          INSERT INTO customer_rating_settings (
            enabled, company_name, company_icon_url, public_base_url, link_valid_hours,
            trigger_source, include_recommendation, questions_json, extension_policies_json
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          normalized.enabled,
          normalized.company_name,
      ...
      console.error('Error saving customer rating settings:', error.message);
      return false;
    }
  }

  getActiveAgentsForExtension(extension) {
    try {
      if (!extension) return [];
      return this.db.prepare(`
        SELECT DISTINCT a.id, a.name, a.extension
        FROM agent_shifts s
        JOIN agents a ON s.agent_id = a.id
        WHERE s.status = 'active'
      ...
      `).all(String(extension));
    } catch (error) {
      console.error('Error getting active agents for extension:', error.message);
      return [];
    }
  }

  createCustomerRatingLink({ phone_number, source, extension = null, call_record_id = null, possible_agent_ids = [], metadata = null }) {
    try {
      const crypto = require('crypto');
      const token = crypto.randomBytes(20).toString('hex');
      const settings = this.getCustomerRatingSettings() || { link_valid_hours: 24 };
      const validHours = Math.max(1, Number(settings.link_valid_hours || 24));
      const expiresAt = new Date(Date.now() + validHours * 60 * 60 * 1000).toISOString();

      this.db.prepare(`
        INSERT INTO customer_rating_links (
          token, phone_number, source, extension, call_record_id,
          possible_agent_ids_json, expires_at, metadata_json
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        token,
        String(phone_number),
        ...
        call_record_id || null,
        JSON.stringify(Array.isArray(possible_agent_ids) ? possible_agent_ids : []),
        expiresAt,
        metadata ? JSON.stringify(metadata) : null
      );

      return this.db.prepare('SELECT * FROM customer_rating_links WHERE token = ?').get(token) || null;
    } catch (error) {
      console.error('Error creating customer rating link:', error.message);
      return null;
    }
  }

  getCustomerRatingTokenDetails(token) {
    try {
      const link = this.db.prepare('SELECT * FROM customer_rating_links WHERE token = ? LIMIT 1').get(String(token));
      if (!link) return null;

      let possibleAgentIds = [];
      try { possibleAgentIds = JSON.parse(link.possible_agent_ids_json || '[]'); } catch (_) {}

      const agents = possibleAgentIds.length > 0
        ? this.db.prepare(
            `SELECT id, name, extension FROM agents WHERE id IN (${possibleAgentIds.map(() => '?').join(',')}) ORDER BY name ASC`
          ).all(...possibleAgentIds)
        : [];

      const settings = this.getCustomerRatingSettings();

      const now = Date.now();
      const expiresMs = new Date(link.expires_at).getTime();
      const isExpired = Number.isFinite(expiresMs) && now > expiresMs;
      const isUsed = !!link.used_at || link.status === 'used';

      if (isExpired && link.status === 'active') {
        this.db.prepare("UPDATE customer_rating_links SET status = 'expired', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(link.id);
      }
      ...
      console.error('Error getting customer rating token details:', error.message);
      return null;
    }
  }

  submitCustomerRating(token, payload = {}) {
    try {
      const details = this.getCustomerRatingTokenDetails(token);
      ...
      if (!Number.isFinite(overall) || overall < 1 || overall > 5) {
        return { success: false, error: 'overall_rating must be between 1 and 5' };
      }

      const possibleAgentIds = (link.possible_agents || []).map((a) => a.id);
      let chosenAgentId = payload.agent_id || null;
      if (chosenAgentId && possibleAgentIds.length > 0 && !possibleAgentIds.includes(chosenAgentId)) {
        chosenAgentId = null;
      }
      if (!chosenAgentId && possibleAgentIds.length === 1) {
        chosenAgentId = possibleAgentIds[0];
      }

      const recommendRating = payload.recommend_rating == null ? null : Number(payload.recommend_rating);
      const comments = payload.comments ? String(payload.comments).slice(0, 2000) : null;
      const answers = payload.answers && typeof payload.answers === 'object' ? payload.answers : {};

      const tx = this.db.transaction(() => {
        this.db.prepare(`
          INSERT INTO customer_rating_submissions (
            link_id, token, phone_number, overall_rating, recommend_rating, comments,
            answers_json, extension, agent_id
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          link.id,
          link.token,
          link.phone_number,
          overall,
          Number.isFinite(recommendRating) ? recommendRating : null,
          comments,
        ...
        this.db.prepare(`
          UPDATE customer_rating_links
          SET status = 'used',
              used_at = CURRENT_TIMESTAMP,
              chosen_agent_id = ?,
              updated_at = CURRENT_TIMESTAMP
          WHERE id = ?
        `).run(chosenAgentId, link.id);

        if (chosenAgentId) {
          this.db.prepare(`
            INSERT INTO agent_ratings (agent_id, rating, comment, rated_by, rating_date)
            VALUES (?, ?, ?, ?, CURRENT_DATE)
          `).run(chosenAgentId, overall, comments, link.phone_number);
        }
      });
      ...
      console.error('Error submitting customer rating:', error.message);
      return { success: false, error: error.message };
    }
  }
