...
class TG400TcpApi extends EventEmitter {
  ...
  handleReceivedSms(data) {
    ...
    this.logger.log('info', `Received SMS on port ${sms.port} from ${sms.sender}`);
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        ports.push({
          ...
          portNumber: portNum,
          status: statusStr,
          isUp: statusStr.includes('Up') && statusStr.includes('Active'),
          isPowerOn: statusStr.includes('Power on'),
