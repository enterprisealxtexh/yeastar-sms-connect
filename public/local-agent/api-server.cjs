...
console.error('[API] ===== END STARTUP TESTS =====')
...
async function sendCallAutoSms(callRecord) {
  try {
    ...
    // Enforce extension filter — admin must explicitly select extensions; empty = no one is monitored
    let allowedExtensions = [];
    try { allowedExtensions = JSON.parse(callAutoSmsConfig.allowed_extensions || '[]'); } catch {}
    if (allowedExtensions.length === 0) {
      logger.debug(` Call auto-SMS: No extensions selected — skipping (select at least one extension to monitor)`);
      return false;
    }
    if (!allowedExtensions.includes(String(callRecord.extension))) {
    ...
    // Check if already in the DB pending queue for this number
    if (db.hasPendingSms && db.hasPendingSms(callerNumber)) {
      logger.info(` Auto-SMS queue duplicate prevention: ${callerNumber} already queued`);
    ...
    if (db.enqueuePendingSms) {
      db.enqueuePendingSms(callerNumber, message, scheduledAtIso);
      logger.info(` Call auto-SMS queued for ${callerNumber}, fires at ${scheduledAtIso} (delay: ${delayMinutes}m)`);
      db.logActivity('call_auto_sms_queued', `SMS queued for ${callerNumber} at ${scheduledAtIso}`, 'info');
    }
...
async function sendEmail(subject, bodyText, { bypassEnabledCheck = false } = {}) {
  try {
    ...
    // Sanitize from address — must be a valid email (or "Name <email>")
    const emailValidator = /[^@\s]+@[^@\s]+\.[^@\s]+/;
    const resolvedFrom = emailValidator.test(emailFrom) ? emailFrom : smtpUser;
    ...
    if (encryption === 'ssl' || (encryption === 'auto' && smtpPort === 465)) {
      secure = true;
    } else if (encryption === 'starttls' || (encryption === 'auto' && smtpPort === 587)) {
      secure = false;
    ...
    logger.info(`📧 SMTP attempt → ${smtpHost}:${smtpPort} secure=${secure} requireTLS=${requireTLS} enc=${encryption} from=${resolvedFrom}`);
...
app.post('/api/test-email', async (req, res) => {
  try {
    ...
    // Sanitize from address — must contain an @ to be valid
    const emailValidator = /[^@\s]+@[^@\s]+\.[^@\s]+/;
    const resolvedFrom = emailValidator.test(emailFrom) ? emailFrom : smtpUser;
    ...
    logger.info(`🧪 Test SMTP → ${smtpHost}:${smtpPort} secure=${secure} requireTLS=${requireTLS} user=${smtpUser} from=${resolvedFrom}`);
    ...
    let hint = '';
    if (/timeout|ECONNREFUSED|ETIMEDOUT/i.test(error.message)) {
      hint = `Connection to ${smtpHost}:${smtpPort} failed. Common causes: (1) VPS/firewall blocks outbound SMTP — try port 465 with SSL encryption, (2) Gmail requires an App Password (not your account password) — enable 2FA then create one at myaccount.google.com/apppasswords.`;
    } else if (/535|authentication/i.test(error.message)) {
      hint = 'Authentication failed. For Gmail: use an App Password, not your main account password. Enable 2FA first at myaccount.google.com/apppasswords.';
    } else if (/certificate|TLS|SSL/i.test(error.message)) {
      hint = 'TLS/SSL handshake failed. Try switching encryption to \'SSL\' with port 465.';
    }
    res.status(400).json({
      success: false,
      error: 'Failed to send test email',
      details: error.message + (hint ? ' — HINT: ' + hint : '')
    });
  }
});
...
app.get('/api/system/update/check', (req, res) => {
  try {
    ...
    const localShort   = localHash.slice(0, 7);
    const localMessage = execSync('git log -1 --format=%s', { cwd: projectDir }).toString().trim();
    const localDate    = execSync('git log -1 --format=%cI', { cwd: projectDir }).toString().trim();

    // Fetch latest from remote to get accurate hash + commit message
    execSync(`git fetch "${authUrl}" ${SYSTEM_UPDATE_BRANCH} --quiet`, {
      cwd: projectDir,
      env: { ...process.env, GIT_TERMINAL_PROMPT: '0' },
      timeout: 15000,
    });
    const remoteHash    = execSync('git rev-parse FETCH_HEAD', { cwd: projectDir }).toString().trim();
    const remoteShort   = remoteHash.slice(0, 7);
    const remoteMessage = execSync('git log FETCH_HEAD -1 --format=%s', { cwd: projectDir }).toString().trim();
    const remoteDate    = execSync('git log FETCH_HEAD -1 --format=%cI', { cwd: projectDir }).toString().trim();
    ...
    return res.json({
      ...
      data: {
        ...
        local:  { hash: localShort,  message: localMessage,  date: localDate  },
        remote: { hash: remoteShort, message: remoteMessage, date: remoteDate },
        updateAvailable,
...
app.post('/api/system/update', (req, res) => {
  ...
  (async () => {
    ...
    try {
      ...
      // Use a detached child process so the reload survives api-server being killed mid-way.
      setTimeout(() => {
        try {
          const { spawn } = require('child_process');
          const child = spawn('pm2', ['reload', 'all', '--update-env'], { detached: true, stdio: 'ignore' });
          child.unref();
        } catch (e) {}
      }, 1000);
    } catch (err) {
